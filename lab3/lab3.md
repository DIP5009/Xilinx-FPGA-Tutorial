...
static long ioctl(struct file *file_p, unsigned int cmd, unsigned long arg)
{
    ...
    int *tmp = (int)lp->base_addr;
	//Xilinx Kernel ioctl cmd 為2時會沒有作用
    switch(cmd){
		/*
        *   這邊自己寫
        *   請參考Lab2
        *   Hint :
        *   CASE :
        *       data[Input Offset] = Input Value;
        *       break;
        *   Input Offset 用法跟Lab2一樣
        */
		default:
...
#ifdef CONFIG_OF
static struct of_device_id sobel_driver_of_match[] = {
	{ .compatible = "xlnx,Sobel-lite-1.0", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, sobel_driver_of_match);
#else
